		...
		if(dirname($dir) == $dropfolderdir) {
			if(is_dir($dir)) {
				...
				while (false !== ($entry = $d->read())) {
					if($entry != '..' && $entry != '.') {
						if(!is_dir($entry)) {
							echo "<tr><td><span style=\"cursor: pointer;\" onClick=\"fileSelected('".$entry."');\">".$entry."</span></td><td align=\"right\">".SeedDMS_Core_File::format_filesize(filesize($dir.'/'.$entry))."</td><td><a href=\""+$entry+"\">Preview</a></td></tr>\n";
						}
					}
				}
				echo "</table>\n";
